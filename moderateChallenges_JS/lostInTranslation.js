...
function getTranslationCode(){
  var key = {
    codel: ["a", "n", "c", "d", "e", "f", "g", "h", "i", "u", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "g", "w", "h", "y", "z", " "],
    english: ["a", "b", "c", "d", "e", "f", "g", "h", "i", "j", "k", "l", "m", "n", "o", "p", "q", "r", "s", "t", "u", "v", "w", "x", "y", "z", " "]
  };
  var sample = "rbc vjnmkf kd yxyqci na rbc zjkfoscdd ew rbc ujllmcp tc rbkso rbyr ejp mysljylc kd kxveddknmc re jsicpdrysi de kr kd eoya kw aej icfkici re zjkr";
  var translated = "the public is amazed by the quickness of the juggler we think that our language is impossible to understand so it is okay if you decided to quit";
  for (var i = 0; i < key.english.length; i++){
    var pos = translated.indexOf(key.english[i]);
    if (pos > -1){
      key.codel[i] = sample[pos];
    }
  }
  return key;
}
...
function translate(text){
  ...
  for (var i = 0; i < text.length; i++){
    char = key.codel.indexOf(text[i]);
    output += key.english[char];
  }
  return output;
}
...
var fs  = require("fs");
var key = getTranslationCode();
fs.readFileSync(process.argv[2]).toString().split('\n').forEach(function (line) {
    if (line !== "") {
        var answer_line = translate(line);
